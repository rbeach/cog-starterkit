...
(function ($, _, Drupal, drupalSettings) {
  ...
  Drupal.geolocation.addMap = function (map) {
    ...
    //SPIDERIFY
    Drupal.geolocation.oms = new OverlappingMarkerSpiderfier(map.googleMap, {
      markersWontMove: true,
      markersWontHide: true,
      basicFormatEvents: false, //To distinguish between spider_format marker values
      keepSpiderfied: true,
  ...
  /**
   * Set/Update a marker on a map WITH SPIDERIFY
   *
   * @param {GeolocationMap} map - The settings object that contains all of the necessary metadata for this map.
   * @param {OverlappingMarkerSpiderifier} - The Overlapping Marker Spiderifier Object
   * @param {GoogleMarkerSettings} markerSettings - Marker settings.
   * @param {Boolean} [skipInfoWindow=false] - Skip attaching InfoWindow.
   * @return {GoogleMarker} - Created marker.
   */
  Drupal.geolocation.setMapMarkerSpider = function (map, markerSettings, skipInfoWindow) {
    map.mapMarkers = map.mapMarkers || [];
    skipInfoWindow = skipInfoWindow || false;
    ...
    if (skipInfoWindow !== true) {
      ...
      google.maps.event.addListener(currentMarker, 'spider_click', function(e){
        if(markerSettings.infoWindowSolitary){
          if (typeof map.infoWindow !== 'undefined') {
            map.infoWindow.close();
          }
    ...
    google.maps.event.addListener(currentMarker, 'spider_format', function(status) {
      if(markerSettings.numberLabel){
        if(status == OverlappingMarkerSpiderfier.markerStatus.SPIDERFIABLE){
          ...
          currentMarker.setLabel({
            text: markerSettings.numberLabel,
            color: "white",
            fontSize: '15px',
          });
        }
    ...
    Drupal.geolocation.oms.addMarker(currentMarker);
